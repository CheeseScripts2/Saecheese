...
local CHEESE_VERSION = "4.2.0"
local CHEESE_BUILD   = "f9ce7f8c"
...
BX.module("boot.log", function(BX)
    ...
    local TRACE_FILE  = "CheeseHub_trace.txt"
    ...
    local PREV_FILE = "CheeseHub_trace_prev.txt"
    if canWrite and type(readfile) == "function" and type(isfile) == "function" then
        pcall(function()
            ...
            if env.__CHEESE_LOG_ROTATED then return end
            env.__CHEESE_LOG_ROTATED = true
...
BX.module("core.profiles", function(BX)
    ...
    local FORMAT = 1
    local DIR = "CheeseHub/profiles"
    local SETTINGS = "CheeseHub/settings.json"
...
BX.module("core.exec", function(BX)
    ...
    --
    --     getgenv().CHEESE_CAPS_DENY = { prompts = true, files = true }
    --
    ...
    local env = (type(getgenv) == "function" and getgenv()) or _G
    local deny = type(env.CHEESE_CAPS_DENY) == "table" and env.CHEESE_CAPS_DENY or {}
...
BX.module("core.config", function(BX)
    return {
        ...
        -- The website's Vercel API (CheeseHubWebsite/api/key) is the authority.
        --
...
BX.module("ui.stats", function(BX)
    ...
    local STROKE_T = 0.45
    local POS_FILE = "CheeseHub_stats_pos.json"
    ...
    --
    -- Downloaded ONCE into CheeseHub/icons; every later load is a local file
    ...
    local ICON_ROOT = "CheeseHub/icons"
...
BX.module("ui.window", function(BX)
    ...
    local env = (type(getgenv) == "function" and getgenv()) or _G
    pcall(function()
        if env.__CHEESE_WINDOW and not env.__CHEESE_WINDOW.unloaded then
            env.__CHEESE_WINDOW:Unload()
        end
    end)
    ...
    sc:delay("visualSkin", 0.35, function()
        ...
        BX.try("window.visualSkin", function()
            ...
            local layer = main:FindFirstChild("CheeseHubParticles")
            if layer then layer:Destroy() end
            layer = Instance.new("Frame")
            layer.Name = "CheeseHubParticles"
                                        ...
                                        if nameStr:find("tag") or nameStr:find("version") or nameStr:find("v4") then
                                            ...
                                            if origBg then
                                                ...
                                                task.delay(0.18, function()
                                                    pcall(function()
                                                        TS_:Create(obj, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { BackgroundColor3 = yellow }):Play()
...
-- =============================================================================
-- FEATURES.ESP.CARDS: V3.1's ESP card, in the CheeseHub tag's visual language
-- =============================================================================
--
...
--
-- THE LOOK IS THE CHEESEHUB TAG'S.
--
    ...
    local function embedFor(e)
        ...
        return {
            ...
            embeds = { {
                ...
                color = 5814783,
                fields = fields,
                footer = { text = "CheeseHub " .. tostring(BX.version) },
...
BX.module("features.misc.appearance", function(BX)
    ...
    --
    -- Measured on the live client: the window shell - Container.<guid>.CheeseHub,
...
BX.module("features.gamethrottle", function(BX)
    ...
    local env = (type(getgenv) == "function" and getgenv()) or _G
    local ENV_KEY = "__CHEESE_THROTTLE"
...
do
    ...
    if not stage("services", true, function()
        ...
        warn("[CHEESE] see CheeseHub_trace.txt")
        return
    end
    ...
    -- custom CheeseHubOverlay GUI below. win is set to a stub so any downstream
    ...
    BX.try("fox.overlay", function()
        ...
        local sg = mk("ScreenGui", {
            Name = "CheeseHubOverlay", ResetOnSpawn = false,
        ...
        --   116092620575995 = yellow anime/banner artwork
        ...
        -- letting the yellow anime artwork remain visible behind the UI.
        ...
        -- looking empty on mobile. It focuses on the yellow/character portion
